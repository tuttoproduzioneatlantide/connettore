...
class LogForAtlantide(models.Model):
    ...
    x_action_timestamp    = fields.Datetime(string="action time", index=True)
    
    x_action_performed_by_user = fields.Integer(string="action was perform by user with this id")
    
    x_is_processed  = fields.Boolean(string="already processed by atlantide", index=True)
    
    x_atlantide_process_failed  = fields.Boolean(string="processed by atlantide with errors", index=True)
